...
var swfobject = function() {
				...
				if (ah.ie && ah.win) {
					aL.attachEvent(ax, function() {
						if (aL.readyState == "complete") {
							aL.detachEvent(ax, arguments.callee);aP();
						}
					});
					if (af == top) {
						(function() {
	...
	function Y() {
		...
		var b = ar(aD);
		b.setAttribute("type", aE);var a = d.appendChild(b);
		if (a) {
			var c = 0;
			(function() {
	...
	function ae(f, d, h, e) {
		...
		if (a) {
			...
			if (ah.ie && ah.win && a.readyState != 4) {
				var g = ar("div");
				h += "SWFObjectNew";g.setAttribute("id", h);a.parentNode.insertBefore(g, a);
	...
	function ao(a) {
		...
		c[2] = parseInt(c[2], 10) || 0;return (b[0] > c[0] || (b[0] == c[0] && b[1] > c[1]) || (b[0] == c[0] && b[1] == c[1] && b[2] >= c[2])) ? true : false;
	}
	function az(b, f, a, c) {
		if (ah.ie && ah.mac) {
			return;
		...
		if (!aH || an != g) {
			var d = ar("style");
			d.setAttribute("type", "text/css");d.setAttribute("media", g);
			aH = e.appendChild(d);
...
SWFUpload.prototype.loadFlash = function() {
	...
	c.innerHTML = this.getFlashHTML();d.parentNode.replaceChild(c.firstChild, d);
...
SWFUpload.prototype.destroy = function() {
	try {
		this.cancelUpload(null, false);var g = null;
		...
		this.movieName = null;return true;
	} catch (i) {
		return false;
	}
};
...
SWFUpload.prototype.setUploadURL = function(b) {
	this.settings.upload_url = b.toString();this.callFlash("SetUploadURL", [ b ]);
};
...
SWFUpload.prototype.setFileTypes = function(d, c) {
	...
	this.settings.file_types_description = c;this.callFlash("SetFileTypes", [ d, c ]);
};
...
SWFUpload.prototype.setFileUploadLimit = function(b) {
	this.settings.file_upload_limit = b;this.callFlash("SetFileUploadLimit", [ b ]);
};
...
SWFUpload.prototype.setAssumeSuccessTimeout = function(b) {
	this.settings.assume_success_timeout = b;this.callFlash("SetAssumeSuccessTimeout", [ b ]);
};
...
SWFUpload.prototype.setButtonTextStyle = function(b) {
	this.settings.button_text_style = b;this.callFlash("SetButtonTextStyle", [ b ]);
};
...
SWFUpload.prototype.fileQueueError = function(e, f, d) {
	e = this.unescapeFilePostParams(e);this.queueEvent("file_queue_error_handler", [ e, f, d ]);
};
...
SWFUpload.prototype.uploadProgress = function(e, f, d) {
	e = this.unescapeFilePostParams(e);this.queueEvent("upload_progress_handler", [ e, f, d ]);
};
SWFUpload.prototype.uploadError = function(e, f, d) {
	e = this.unescapeFilePostParams(e);this.queueEvent("upload_error_handler", [ e, f, d ]);
};
SWFUpload.prototype.uploadSuccess = function(d, e, f) {
	d = this.unescapeFilePostParams(d);this.queueEvent("upload_success_handler", [ d, e, f ]);
};
SWFUpload.prototype.uploadComplete = function(b) {
	b = this.unescapeFilePostParams(b);this.queueEvent("upload_complete_handler", b);
};
...
SWFUpload.prototype.debugMessage = function(h) {
	if (this.settings.debug) {
		...
		if (typeof h === "object" && typeof h.name === "string" && typeof h.message === "string") {
			...
			f = g.join("\n") || "";
			g = f.split("\n");
			f = "EXCEPTION: " + g.join("\nEXCEPTION: ");SWFUpload.Console.writeLine(f);
		} else {
...
SWFUpload.Console.writeLine = function(g) {
	...
	try {
		...
		if (!e) {
			...
			e.style.fontFamily = "monospace";e.setAttribute("wrap", "off");
			e.wrap = "off";
			e.style.overflow = "auto";
			e.style.width = "700px";
			e.style.height = "350px";
			e.style.margin = "5px";f.appendChild(e);
		}
		e.value += g + "\n";
	...
	var b = {
		...
		cancel : function(d, f) {
			...
			this.each(function() {
				...
				if (e[0]) {
					...
					$item = c(k);i.cancelUpload($item.attr("id"));$item.find(".data").removeClass("data").html(" - Cancelled");$item.find(".uploadify-progress-bar").remove();$item.delay(1000).fadeOut(500, function() {
						c(this).remove();
					});
				}
			});
		...
		upload : function() {
			...
			this.each(function() {
				...
				if (d[0]) {
					if (d[0] == "*") {
						e.queueData.uploadSize = e.queueData.queueSize;e.queueData.uploadQueue.push("*");e.startUpload();
					} else {
	...
	var a = {
			...
			for (var g in this.queueData.files) {
				...
				if (f.uploaded != true && f.name == h.name) {
					...
					if (!e) {
						...
						c("#" + f.id).remove();this.cancelUpload(f.id);this.queueData.filesReplaced++;
					}
				}
			}
			...
			if (c.inArray("onUploadComplete", g.overrideEvents) < 0) {
				if (g.removeCompleted) {
					...
					case SWFUpload.FILE_STATUS.COMPLETE:
						...
						break;case SWFUpload.FILE_STATUS.ERROR:
						if (!g.requeueErrors) {
							setTimeout(function() {
								if (c("#" + f.id)) {
									d.queueData.queueSize -= f.size;
		...
		onUploadError : function(e, i, h) {
			...
			case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
				...
				alert("The upload limit has been reached (" + h + ").");g = "Exceeds Upload Limit";
				break;case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:
				g = "Failed";
				break;case SWFUpload.UPLOAD_ERROR.SPECIFIED_FILE_ID_NOT_FOUND:
				break;case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:
				g = "Validation Error";
				break;case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
				g = "Cancelled";this.queueData.queueSize -= e.size;this.queueData.queueLength -= 1;
				...
				c.ajax({
					...
					success : function(h) {
						if (h == 1) {
							...
							if (!g) {
								this.cancelUpload(d.id);c("#" + d.id).remove();
