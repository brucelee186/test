...
$(function(){
	$('#dtg_purchase').datagrid({
		fit : true,
		border : false,
		url : '${pageContext.request.contextPath}/office/reimbursement/reimbursement/doSearchForReport.do',
		idField : 'id',
		...
		columns : [ [/* {
			...
			formatter : function(value, row, index) {
				if(row.listReimbursementItem){
					return row.listReimbursementItem[0].category1Name;
				}
			}
		},{
			title : '次类别',
			...
			formatter : function(value, row, index) {
				if(row.listReimbursementItem){
					return row.listReimbursementItem[0].category2Name;
				} else if(value){
					return value;
				}
		...
		toolbar : [ {
			...
			text : '导出  Excel',
			handler : function() {
				$('#frm_search').attr('action', '${pageContext.request.contextPath}/office/reimbursement/reimbursement/doExportReimbursementReport.do');
...
function submitSearchForm() {
	...
	if (opt.url == undefined || opt.url == null || opt.url == '') {
		opt.url = '${pageContext.request.contextPath}/office/reimbursement/reimbursement/doSearchForReport.do';
...
<body class="easyui-layout" data-options="fit : true,border : false">
	<div data-options="region:'center',title:'报销统计列表',border:false">
	...
	<div data-options="region:'north',title:'搜索',border:true" style="height: 90px;overflow: hidden;" align="center">
		...
		<%-- <input  name="statusGroup" value="${purchase.statusGroup}" type="hidden"/> --%>
			<table class="mtftable">
					...
					<td>:</td>
					<td><input  name="customerId" class="easyui-combotree" style="width:180px;" 
					data-options="url : '${pageContext.request.contextPath}/maintenance/division/doSearch.do', valueField : 'id', textField : 'name', parentField : 'pid', lines : true, multiple : true "/></td> --%>
					<th align="right" style="width: 80px;">报销状态</th>
					<td>:</td>
					<td><select  name="reimbursementStatus" class="easyui-combobox" style="width:180px" data-options="panelHeight:'auto',panelMaxHeight:'200',editable:false ">
							<option value="1">已审核通过</option>
							<option value="2">报销中</option>
						</select></td>
