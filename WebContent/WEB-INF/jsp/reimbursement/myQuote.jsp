	...
	$('#dtg_purchase').datagrid({
		...
		toolbar : [{
				...
				handler : function() {
					...
					$('#dlg_purchase').dialog('setTitle', '添加报价').dialog('open').dialog('center');
					//隐藏输入框边框
			...
			, '-',{
						...
						try {
							...
							if (r.success) {
								 $('#frm_purchase').form('load', {
									 ...
									 'quoteAmount' : r.obj.itemlist[0].quoteAmount,
									 'quoteCurrencyId' : r.obj.dbPurchase.quoteCurrencyId,
									 'exchangeRate' : r.obj.dbPurchase.exchangeRate,
									 'totalRmbAmount' : r.obj.dbPurchase.totalRmbAmount
								});
								//联动相应的类别项
								...
								//隐藏和显示相应的汇率框
								if(r.obj.dbPurchase.quoteCurrencyId == 1){
				...
				/* 	var $dtg = $('#dtg_purchaseItem');
					...
					$('#dlg_purchase').dialog('setTitle', '编辑报价').dialog('open').dialog('center');
			...
			, '-',  {
				...
				handler : function() {
					...
					if(checkedRows[0].status == 3){
						$.messager.alert('<s:message code="common.dialog.tip" />', '已提交报价！', 'warning');
						return;
					}
					
					if(checkedRows[0].status != 0){
						$.messager.alert('<s:message code="common.dialog.tip" />', '只有【草稿】状态可以提交报价！', 'warning');
						return;
					}
					$.messager.confirm('<s:message code="common.confirm" />', '确认提交报价？', function(rr) {
						if(rr){
							$.post('${pageContext.request.contextPath}/office/reimbursement/purchase/doEditStatusQuote.do', {id : checkedRows[0].id}, function(result){
								$.messager.progress('close');// hide progress dialog
								try {
									var j = result;
									if (j.success) {
										$.messager.alert('<s:message code="common.tip" />', '提交报价成功！', 'info', function(rr) {
			...
			, '-',  {
				...
				handler : function() {
					...
					if(checkedRows[0].status != 0){
						$.messager.alert('<s:message code="common.dialog.tip" />', '只有【草稿】状态可以取消报价！', 'warning');
						return;
					}
					$.messager.confirm('<s:message code="common.confirm" />', '确认取消报价？', function(rr) {
						if(rr){
							$.post('${pageContext.request.contextPath}/office/reimbursement/purchase/doDelete.do', {id : checkedRows[0].id}, function(result){
								...
								try {
									var j = result;
									if (j.success) {
										$.messager.alert('<s:message code="common.tip" />', '取消报价成功！', 'info', function(rr) {
											$('#dtg_purchase').datagrid('reload');
										});
	...
	$('#nbx_exchangeRate').numberbox({
		inputEvents:$.extend({},$.fn.numberbox.defaults.inputEvents,{
			keyup:function(e){
				...
				var quoteAmount = $('#nbx_quoteAmount').numberbox('getValue');
...
function doSubmit(param) {
	...
	var quoteCurrencyId = $('#cbx_quoteCurrencyId').combobox('getValue');
	var exchangeRate = $('#nbx_exchangeRate').numberbox('getValue');
	var totalRmbAmount = $('#nbx_totalRmbAmount').numberbox('getValue');
	
	if($('#hid_editStatus').val() == 1){
		//1添加
		url = '${pageContext.request.contextPath}/office/reimbursement/purchase/doAddForQuote.do';
		//公用信息
		var form = {
			'status' : param,
		...
		var arr = [];
			...
			var row = {
					...
					'quoteAmount' : $('#nbx_quoteAmount').numberbox('getValue')
				};
			arr.push(row);
	}else{
		//2编辑
		url = '${pageContext.request.contextPath}/office/reimbursement/purchase/doEditForQuote.do';
		var checkedRows = $('#dtg_purchase').datagrid('getSelections');
		//公用信息
		var form = {
...
<body class="easyui-layout" data-options="fit : true,border : false">
	...
	<div id="dlg_purchase" class="easyui-dialog" style="width:510px;height:315px;" data-options="resizable:true,modal:true,closed: true, 
		...
		buttons: [{text:'<s:message code="保存为草稿" />', iconCls:'icon-save', handler:function(){doSubmit(0);}},
				  {text:'<s:message code="保存并提交" />', iconCls:'ext-icon-drive_go', handler:function(){doSubmit(3);}}]">
		<form id="frm_purchase" method="post">
		<input id="hid_editStatus" name="editStatus" type="hidden"/>
		<div>
		<table class="mtftable" align="center">
		<tr><th align="right" style="width: 70px;">报价部门</th>
			<td>:</td>
		...
		<table class="mtftable" align="center">
			<tbody>
				<tr>
				<th align="right" style="width: 70px;">估算金额</th>
				<td>:</td>
				<td><input id="nbx_quoteAmount" name="quoteAmount" type="text" class="easyui-numberbox" style="width:150px" data-options=" precision:2, validType:'digitalCompare[0, 300000]', required:true " /></td>
				<th align="right"style="width: 50px;">货币</th>
				<td>:</td>
				<td><select id="cbx_quoteCurrencyId" name="quoteCurrencyId" class="easyui-combobox" style="width:150px" data-options="panelHeight:'auto', panelMaxHeight:'200',editable:false, required:true,
					onSelect: function(rec){ 
				...
				<tr id="tr_exchangeRate">
					<th align="right" style="width: 70px;">汇率</th>
					<td>:</td>
					<td><input id="nbx_exchangeRate" name="exchangeRate" value="1.0000" class="easyui-numberbox" style="width:150px" data-options=" precision:4, validType:'digitalCompare[0, 100]', required:true " /></td>
					
					<th align="right" style="width: 70px;">换算总金额</th>
					<td>:</td>
					<td align="center" valign="middle"><div style="width:95%"><input id="nbx_totalRmbAmount" name="totalRmbAmount" type="text" class="easyui-numberbox"  readonly="readonly" style="width:150px" data-options="min:0, precision:2, required:true " /></div></td>
				</tr>
				...
				<td>:</td>
				<td colspan="4"><input id="txt_description" name="description" class="easyui-textbox" style="width:390px;height:100px;" maxlength="255" data-options="multiline:true,validType:'length[0,255]', required : true" /></td>
				</tr>
			</tbody>
		</table>
		</div>
