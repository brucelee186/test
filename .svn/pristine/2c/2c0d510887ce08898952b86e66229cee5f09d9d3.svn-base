...
<script type="text/javascript">
	$(function() {
		$('#datagrid').datagrid({
			fit : true,
			border : false,
			url : '${pageContext.request.contextPath}/admin/role/doSearch.do',
			pagination : false,
			idField : 'id',
			sortName : 'name',
			sortOrder : 'asc',
			...
			toolbar : [{
				...
				handler : function() {
					$('#form').attr('action', '${pageContext.request.contextPath}/admin/role/doAdd.do');
					$('#form').form('load', {
						id : '',
						name : '',
						level : 1,
				...
				handler : function() {
					...
					$('#form').attr('action', '${pageContext.request.contextPath}/admin/role/doEdit.do');
					$('#form').form('load', {
						id : checkedRows[0].id,
				...
				handler : function() {
					...
					var content = formatString('<iframe src="admin/role/toEditActions.do?id={0}" frameborder="0" style="border : 0; width : 100%; height : 99%;"></iframe>', checkedRows[0].id);
				...
				handler : function() {
					...
					$.messager.confirm('<s:message code="common.dialog.confirm" />', '确认删除?', function(r) {
						if (r) {
							$.post("${pageContext.request.contextPath}/admin/role/doDelete.do", { id : checkedRows[0].id }, function(result) {
								try {
...
<body class="easyui-layout" data-options="fit : true,border : false">
	<div id="dialog" class="easyui-dialog" style="width:400px;height:260px;" data-options="resizable:true,modal:true,closed: true, buttons: '#dialog_buttons'">
		<form id="form" method="post">
			<input type="hidden" name="id" />
			<table class="tableForm">
				<tr>
					<th style="width: 150px;">Name : </th>
					<td><input class="easyui-validatebox" name="name" type="text" style="width: 250px" data-options="required:true" /></td>
				</tr>
				<tr>
					<th>Level : </th>
	...
	<div data-options="region:'center',title:'System Role List',border:false" align="center">
