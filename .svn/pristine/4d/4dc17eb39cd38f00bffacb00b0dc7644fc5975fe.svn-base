...
public class RoleAdminController {
	...
	private static final Logger		logger	= Logger.getLogger(RoleAdminController.class);
	...
	public DataGrid<RoleImpl> doSearch(PageForm form, HttpSession session) throws PmException {
		...
		try {
			List<RoleImpl> list = this.roleService.listAll(sessionInfo.getUserId(), form, 1, null);
	...
	public DataGrid<RoleImpl> doSearch4User(PageForm form, HttpSession session) throws PmException {
		...
		DataGrid<RoleImpl> result = new DataGrid<RoleImpl>();
		try {
			List<RoleImpl> list = this.roleService.listAllAvailable(sessionInfo.getUserId(), form, null);
			if (list != null && !list.isEmpty()) {
				result.setTotal(list.size());
				result.setRows(list);
			}
