				...
				handler : function() {
					...
					for(var i = 0; i < checkedRows.length; i++){
						if(checkedRows[i].status == 2){
							$.messager.alert('<s:message code="common.dialog.tip" />', '已完成审批！', 'warning');
							return;
						}
						if(checkedRows[i].status != 1){
							$.messager.alert('<s:message code="common.dialog.tip" />', '只有【提交】状态可以编辑！', 'warning');
							return;
						}
					...
					if(checkedRows.length == 1){
						...
						$('#dlg_approve').dialog('setTitle', '审批申请').dialog('open').dialog('center');
					}else{
						//check多条时的报销方式
						$.messager.confirm('<s:message code="common.confirm" />', '确认批量审批？', function(rr) {
							if(rr){
								$.post('${pageContext.request.contextPath}/office/reimbursement/purchase/doBatchApproveLv1.do', {ids : idsStr}, function(result){
...
function doSubmit(param) {
	...
	if(param == 8){
		...
		url = '${pageContext.request.contextPath}/office/reimbursement/purchase/doApproveLv1.do';
	}
