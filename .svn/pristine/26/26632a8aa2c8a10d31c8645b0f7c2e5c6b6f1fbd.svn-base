...
public class Test {
	...
	private static void select(Person p, IdCard i) {
		Session s = new Configuration().configure("framework/hibernate/one2one/uniqueForeignKey/hiberante.cfg.xml").buildSessionFactory().openSession();
...
//		Configuration config = new Configuration().addResource("framework/hibernate/one2one/hiberante.cfg.xml");
		...
		config.configure("framework/hibernate/one2one/uniqueForeignKey/hiberante.cfg.xml");
