...
<script type="text/javascript" charset="utf-8">
	$(function() {
		$('#form').form({
			url : '${pageContext.request.contextPath}/admin/role/doEditActions.do',
			onSubmit: function() {
		...
		$('#datagrid_role_action').datagrid({
			url : '${pageContext.request.contextPath}/admin/role/doSearchActions.do',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : false,
			...
			toolbar : [{
				...
				handler : function() {
					var checkedRows = $('#datagrid_role_action').datagrid('getChecked').slice(0);
					if (checkedRows == null || checkedRows == undefined || checkedRows.length == 0) {
						return;
					}
					var i = 0;
					for (i = 0; i < checkedRows.length; i ++) {
						var rowIndex = $('#datagrid_role_action').datagrid('getRowIndex', checkedRows[i]);
						$('#datagrid_role_action').datagrid('deleteRow', rowIndex);
					}
				}
			}, '-', {
				iconCls : 'icon-save',
				...
				handler : function() {
					var actionIds='';
					var resources = $('#datagrid_role_action').datagrid('getRows');
		...
		$('#datagrid_action').datagrid({
			...
			toolbar : [{
				...
				handler : function() {
					...
					var rows = $('#datagrid_role_action').datagrid('getRows');
					var countSkiped = 0;
					var countAdded = 0;
					var isExists = false;
					for (var i = 0; i < checkedRows.length; i ++) {
						...
						if (isExists == false) {
							$('#datagrid_role_action').datagrid('appendRow', checkedRows[i]);
							countAdded ++;
						}
					}
		...
		$('#datagrid_role_action').datagrid('loadData', eval('${actionList}'));
	});
</script>
</head>
<body class="easyui-layout" data-options="fit : true,border : false">
	<div data-options="region:'center',title:'Role [${role.name}] Actions',border:false" align="center">
		...
		<table id="datagrid_role_action"></table>
	</div>
	<div data-options="region:'east',border:false,split:true,title:'Available Actions'" style="width: 450px; overflow: hidden;">
