	...
	$.fn.portal.methods = {
		...
		addColumn: function (jq) {
			return jq.each(function () {
				...
				tr = portal.find(">table>tbody>tr"), td = $("<td></td>").addClass("portal-column-td").appendTo(tr),
		...
		removeColumn: function (jq, index) {
			return jq.each(function () {
				...
				var tr = $(this).find(">table>tbody>tr"), width = parseFloat(1) / (columnWidths.length - 1);
				columnWidths.pop();
				for (var i = 0; i < columnWidths.length; i++) { columnWidths[i] = width; }
