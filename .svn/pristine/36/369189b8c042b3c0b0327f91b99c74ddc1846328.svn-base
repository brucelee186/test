...
<script type="text/javascript">
	...
	function initAdmDatagrid(){
		$('#datagrid').datagrid({
			...
			<c:if test="${tagPageCode == '16200500'}">
			{	
					title : '操作',  
					field : 'addIp',       
					width : 50,  
					formatter : function(value, row, index) {
						var str = '';
						str = '<a id="input_opt_' + index + '" type="button" value="查看" required="true" href="javascript:countWorkExtraDetail4(' + index + ')" >查看</a>';
						return str;
					}
				},		
			</c:if>	
	...
	function countWorkExtraDetail(){
		...
		var url = "${pageContext.request.contextPath}/ReportServer?reportlet=showCountWorkExtraDetailV4.cpt&__bypagesize__=false&serverIp=${pageContext.request.contextPath}&applyMonth=" + applyMonth + "&attenCollectDateStart=" + attenCollectDateStart + "&attenCollectDateEnd=" + attenCollectDateEnd;
		window.location.href = url;
	}
	//  加班统计明细 
	function countWorkExtraDetail2(){
		var checkedRows = $('#datagrid').datagrid('getSelections');
		if (checkedRows == null || checkedRows == undefined || checkedRows.length == 0) {
			$.messager.alert('<s:message code="common.dialog.tip" />', '<s:message code="office.dialogue.pleaseSelectOne" />', 'warning');
			return;
		...
		var attenCollectDateStart =  checkedRows[0].attenCollectDateStart;
		var attenCollectDateEnd =  checkedRows[0].attenCollectDateEnd;
		var url = "${pageContext.request.contextPath}/ReportServer?reportlet=showCountWorkExtraDetailV5.cpt&__bypagesize__=false&serverIp=${pageContext.request.contextPath}&applyMonth=" + applyMonth + "&attenCollectDateStart=" + attenCollectDateStart + "&attenCollectDateEnd=" + attenCollectDateEnd;
		window.location.href = url;
	}
	
	//  加班统计明细 
	function countWorkExtraDetail3(){
		var checkedRows = $('#datagrid').datagrid('getSelections');
		if (checkedRows == null || checkedRows == undefined || checkedRows.length == 0) {
			$.messager.alert('<s:message code="common.dialog.tip" />', '<s:message code="office.dialogue.pleaseSelectOne" />', 'warning');
			return;
		...
		var url = "${pageContext.request.contextPath}/ReportServer?reportlet=showCountWorkExtraDetailV5.cpt&__bypagesize__=false&serverIp=${pageContext.request.contextPath}&applyMonth=" + applyMonth + "&attenCollectDateStart=" + attenCollectDateStart + "&attenCollectDateEnd=" + attenCollectDateEnd;
		//window.location.href = url;
		var content = '<iframe src="${pageContext.request.contextPath}/ReportServer?reportlet=showCountWorkExtraDetailV5.cpt&__bypagesize__=false&serverIp=${pageContext.request.contextPath}&applyMonth=' + applyMonth + '&attenCollectDateStart=' + attenCollectDateStart + '&attenCollectDateEnd=' + attenCollectDateEnd + '&divisions=${divisionIds}&serverIp=${pageContext.request.contextPath}' + '&tagPageCode=${tagPageCode}" frameborder="0" style="border : 0; width : 100%; height : 99%;"></iframe>'; 
	...
	//  加班统计明细 
	function countWorkExtraDetail4(index){
		var row = $('#datagrid').datagrid('getData').rows[index];
		var applyMonth = row.applyMonth;
		var attenCollectDateStart =  row.attenCollectDateStart;
		var attenCollectDateEnd =  row.attenCollectDateEnd;
		var url = "${pageContext.request.contextPath}/ReportServer?reportlet=showCountWorkExtraDetailV5.cpt&__bypagesize__=false&serverIp=${pageContext.request.contextPath}&applyMonth=" + applyMonth + "&attenCollectDateStart=" + attenCollectDateStart + "&attenCollectDateEnd=" + attenCollectDateEnd;
		//window.location.href = url;
		var content = '<iframe src="${pageContext.request.contextPath}/ReportServer?reportlet=showCountWorkExtraDetailV5.cpt&__bypagesize__=false&serverIp=${pageContext.request.contextPath}&applyMonth=' + applyMonth + '&attenCollectDateStart=' + attenCollectDateStart + '&attenCollectDateEnd=' + attenCollectDateEnd + '&divisions=${divisionIds}&serverIp=${pageContext.request.contextPath}' + '&tagPageCode=${tagPageCode}" frameborder="0" style="border : 0; width : 100%; height : 99%;"></iframe>'; 
			top.layout_center_addTab({
			...
			<tr>
				...
				<td><a href="javascript:void(0)" class="l-btn l-btn-plain"
					onclick="javascript:countWorkExtraDetail()"><span class="l-btn-left"><span
							class="l-btn-text ext-icon-chart_curve l-btn-icon-left">加班统计V1</span></span></a></td>
							
				<td><a href="javascript:void(0)" class="l-btn l-btn-plain"
					onclick="javascript:countWorkExtraDetail2()"><span class="l-btn-left"><span
							class="l-btn-text ext-icon-chart_curve l-btn-icon-left">加班统计V2</span></span></a></td>
				...
				<c:if test="${tagPageCode == '16200500'}">
				<td><a href="javascript:void(0)" class="l-btn l-btn-plain"
					onclick="javascript:countWorkExtraDetail3()"><span class="l-btn-left"><span
